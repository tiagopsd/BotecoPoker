...
{
    public class ImprimeComprovante : ImpressaoBase
    {
        private ComprovanteModel comprovanteModel;
        ...
        private void EventoCIS(object sender, PrintPageEventArgs ev)
        {
        ...
        private void EventoEpson(object sender, PrintPageEventArgs ev)
        {
            ...
            if (comprovanteModel.Vendas.Count() > 0)
            {
                ...
                foreach (var venda in comprovanteModel.Vendas)
                {
                    ...
                    {
                        ev.Graphics.DrawString($"{preVenda.Produto.Nome}: {preVenda.Produto.Valor.ToString("c2")} QTD: {preVenda.Quantidade}", pdvFont, Brushes.Black, 15, currentUsedHeight, new StringFormat());
                        size = ev.Graphics.MeasureString("X", spaceFonte);
                        currentUsedHeight += size.Height;
                    }
